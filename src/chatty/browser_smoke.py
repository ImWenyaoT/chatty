...
DEFAULT_E2E_DATABASE = ".cache/browser-e2e.sqlite"
...
def browser_smoke_database_path() -> Path:
    """解析 `CHATTY_E2E_DATABASE`（相对路径按仓库根解析；未设置退回旧默认）。"""
    raw = os.environ.get("CHATTY_E2E_DATABASE") or DEFAULT_E2E_DATABASE
    path = Path(raw)
...
def create_app() -> FastAPI:
    ...
    """
    database_path = browser_smoke_database_path()
