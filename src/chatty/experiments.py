...
class GroupStats:
    ...
    recommendations: int = 0  # 这一组处理过多少次请求
    request_successes: int = 0
    request_failures: int = 0
    total_latency_ms: float = 0.0  # 累计耗时，除以 recommendations 得平均延迟
...
class ExperimentMetrics:
    ...
    def assign(self, user_id: str) -> ExperimentGroup:
        ...
        用哈希而不是随机数，是为了保证同一个用户每次请求都落在同一组——
        否则用户这次看到 A 策略、下次看到 B 策略，实验结论就没意义了。
        取哈希前 8 字节对 2 取模，天然是 50/50 分桶。
        """
