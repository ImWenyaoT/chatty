// 朴素金标回归运行器（单 lane，docs/agentic-search-design.md §5）。
// 用法：
 ...
 */
function checkExpectations(expect: ExpectBlock, turn: TurnOutcome, prevAnswer?: string): string[] {
...
async function runAllScenarios(scenarios: Scenario[], concurrency = 6): Promise<ScenarioResult[]> {
  return mapWithConcurrency(scenarios, Math.max(1, concurrency), async (scenario) => {
    try {
