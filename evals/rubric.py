...
对应《深入理解 AI Agent》第 6.1 节的 Rubric 示例与 6.3 节"可验证性保障"。

教材在客服退款例子里用了四个维度：操作正确性、政策合规性、信息完整性、
幻觉检测（否决项）。映射到电商推荐场景：
...
因为它评的 Agent 没有运行时守卫。Chatty 不同——工具是否按序调用是
Harness 的**运行时前置条件**（agent.py 里 `used_tools != TOOL_NAMES` 直接抛错），
所以"成功返回"本身就蕴含了"五个工具已按序调用"，再单独评一维恒为真、没有区分度；
而违规的情况会以 `required_tools_not_used` 错误码出现，已被拒绝路径覆盖。
这正是"守卫"和"评分"的区别：守卫让违规无法发生，评分只是记录违规发生了。

**否决项**的含义（教材原话）：它与质量正交——一个流畅、详尽的回答如果包含
