...
function DistributionChart({
  ...
  buckets: ReturnType<typeof scoreDistribution>
  total: number
}) {
      ...
      {buckets.map((b) => {
        ...
        return (
          <div key={b.label} className="dist-row">
            <div className="dist-range" style={{ color: b.color }}>
              {b.label}
            </div>
...
function TrendSection({ customers }: { customers: CustomerListItem[] }) {
  ...
  const peakDay = buckets.length
    ? buckets.reduce((a, b) => (b.avg > a.avg ? b : a), buckets[0])
    : null
  const worstDay = buckets.length
    ? buckets.reduce((a, b) => (b.avg < a.avg ? b : a), buckets[0])
    : null
        ...
        <div className="verdict-body">
          ...
          {verdict.kind !== 'nodata' && (
            <div className="verdict-meta">
              <span>
                <em>前期均分</em> {verdict.prevAvg.toFixed(2)}
              </span>
              <span className="verdict-meta-arrow">→</span>
              <span>
                <em>近期均分</em>{' '}
                <b style={{ color: scoreColor(verdict.latestAvg) }}>
                  {verdict.latestAvg.toFixed(2)}
                </b>
              </span>
            </div>
          )}
        ...
        <div className="verdict-stats">
          ...
          <div className="vs-item">
            ...
            <div className="vs-value">
              {buckets.length ? (maxScore - minScore).toFixed(1) : '-'}
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="trend-title">
            ...
            {peakDay && worstDay && buckets.length > 1 && (
              <span className="trend-extremes">
                <em>高点</em> <b style={{ color: '#34d399' }}>{peakDay.avg.toFixed(2)}</b> ·{' '}
                {peakDay.label}
                <span className="trend-sep">/</span>
                <em>低点</em> <b style={{ color: '#f87171' }}>{worstDay.avg.toFixed(2)}</b> ·{' '}
                {worstDay.label}
              </span>
            )}
          </div>
...
function TiltCard({
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: 纯视觉倾斜特效，鼠标悬浮装饰，无交互语义
    <div
      ref={ref}
    ...
    <div className="cards">
      <TiltCard>
        ...
        <div className="card-sub">
          {config?.chatModel} → eval {config?.evaluatorModel}
        </div>
      </TiltCard>
      <TiltCard>
      ...
      <TiltCard>
        ...
        <div
          ...
          style={{ WebkitTextFillColor: scoreColor(avgScore), color: scoreColor(avgScore) }}
        >
          {avgScore.toFixed(2)}
        </div>
        <div className="card-sub">基于 {current?.count ?? 0} 条评估</div>
        {versionScores.length > 1 && (
          <MiniSpark values={versionScores} color={scoreColor(avgScore)} />
        )}
      </TiltCard>
      <TiltCard>
            ...
            <div key={v.version} className="version-row">
              ...
              <div className="version-bar">
                <div
                  ...
                  style={{
                    width: `${scorePct}%`,
                    background: `linear-gradient(90deg, ${scoreColor(v.avgScore)}, rgba(34,211,238,0.9))`,
                  }}
                />
              ...
              <div className="version-meta">
                <div className="mini-stat">
                  ...
                  />
                  <span>volume</span>
                  <span className="mini-stat-bar">
                    <span style={{ width: `${countPct}%`, background: '#818cf8' }} />
                  </span>
                ...
                <div className="mini-stat">
                  ...
                  <span>low (&lt;6)</span>
                  <span className="mini-stat-bar">
                    <span style={{ width: `${lowPct}%`, background: '#f87171' }} />
                  </span>
                ...
                <div className="mini-stat">
                  ...
                  <span>errors</span>
                  <span className="mini-stat-bar">
                    <span style={{ width: `${errPct}%`, background: '#a78bfa' }} />
                  </span>
...
function FrequencyList({
  ...
  title: string
  items: { issue?: string; suggestion?: string; count: number }[]
}) {
    ...
    <div className="conv-list">
      ...
      {flat.map(({ customer, pm, lastReview }, i) => {
        const isSelected =
          ...
          selected?.conversationId === pm.conversationId
        const score = lastReview?.score ?? 0
        const isError = !!lastReview?.error
        const isLow = !isError && lastReview && score > 0 && score < 6
        return (
          // biome-ignore lint/a11y/useSemanticElements: 列表项含块级布局，button 不能包块级内容；role+tabIndex+onKeyDown 已保证键盘可达
          <div
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' || e.key === ' ') onSelect(customer, pm)
            }}
          ...
          >
            ...
            <div className="conv-sub">
              {pm.productId} · <span className="mono">{pm.reviews.length}</span> reviews
            </div>
...
function ConversationDetail({
  ...
  return (
    <div className="detail">
      ...
      {profile &&
        (profile.heightCm || profile.weightKg || profile.rentalPeriod || profile.productIntent) && (
          <div className="profile-card">
            {profile.productIntent?.currentProductText && (
              <span>意向: {profile.productIntent.currentProductText}</span>
            )}
            {profile.heightCm && <span>身高: {profile.heightCm}cm</span>}
            {profile.weightKg && <span>体重: {profile.weightKg}kg</span>}
            {profile.rentalPeriod && (
              <span>
                档期: {profile.rentalPeriod.startDate ?? '?'} ~{' '}
                {profile.rentalPeriod.endDate ?? '?'}
              </span>
            )}
            {profile.orchestration?.stage && <span>阶段: {profile.orchestration.stage}</span>}
            {profile.orderPlacement?.orderNo && (
              <span>订单号: {profile.orderPlacement.orderNo}</span>
            )}
          </div>
        )}
...
export default function App() {
  ...
  } = useAsync(() => fetchCustomers(1, 200), [])
  const [selected, setSelected] = useState<
    { customerId: string; conversationId: string } | undefined
  >()
  ...
  const selectedPm = selectedCustomer?.productMemories.find(
    (pm) => pm.conversationId === selected?.conversationId,
  )
  ...
  const onReEvaluate = async () => {
    ...
    try {
      await triggerReEvaluate(
        ...
        selectedPm.productId,
        selectedPm.conversationId,
      )
    ...
    <div className="app">
      <header className="app-header">
        ...
        <div className="app-header-right">
          ...
          <button type="button" onClick={doRefresh} className={isLoading ? 'btn-spin' : ''}>
      ...
      {summary && summary.promptVersions.length > 1 && (
        <div className="filter-bar">
          <span className="filter-label">过滤 · Filter</span>
      ...
      <div className="two-col">
        <div className="sidebar">
          <ConversationList
            ...
            selected={selected}
            onSelect={(c, pm) =>
              setSelected({ customerId: c.customerId, conversationId: pm.conversationId })
            }
