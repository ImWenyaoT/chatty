# harness 版金标场景（B4，docs/agentic-search-design.md §5.3–§5.4）

本目录是 `pnpm eval -- --target harness` 读取的场景集。legacy 版原文件
（`../golden/`）保持不动，直到 B5/R4 退役。每个 YAML 头部注释逐场景说明了
与 legacy 版的语义等价性（映射依据 §5.3：质量不变量→行为断言；可映射的
