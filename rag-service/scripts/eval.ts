...
import { appendConversationMemory, flushPendingReviews, getProductMemory } from '../src/memory-store.js';
...
function checkProfile(profile: ReturnType<typeof getProductMemory> extends Promise<infer T> ? T : never, expect: ExpectProfile): string[] {
  ...
  if (expect.rentalPeriod) {
    if (expect.rentalPeriod.startDate !== undefined && !matchExpect(cp.rentalPeriod?.startDate, expect.rentalPeriod.startDate)) {
      failures.push(`rentalPeriod.startDate expected=${expect.rentalPeriod.startDate} got=${cp.rentalPeriod?.startDate}`);
    }
    if (expect.rentalPeriod.endDate !== undefined && !matchExpect(cp.rentalPeriod?.endDate, expect.rentalPeriod.endDate)) {
      failures.push(`rentalPeriod.endDate expected=${expect.rentalPeriod.endDate} got=${cp.rentalPeriod?.endDate}`);
    }
  }
  if (expect.productIntent?.currentProductText !== undefined && !matchExpect(cp.productIntent?.currentProductText, expect.productIntent.currentProductText)) {
    failures.push(`productIntent.currentProductText expected=${expect.productIntent.currentProductText} got=${cp.productIntent?.currentProductText}`);
  }
  return failures;
}
...
async function runScenario(scenario: Scenario): Promise<ScenarioResult> {
  ...
  for (const step of scenario.steps) {
    ...
    const pm = await getProductMemory(scenario.customerId, scenario.productId, scenario.conversationId);
  ...
  const finalMemory = await getProductMemory(scenario.customerId, scenario.productId, scenario.conversationId);
  const systemReviews = (finalMemory?.reviews ?? []).filter((r) => r.source === 'system' && r.score > 0);
...
async function compareBaseline(baselineTag: string, results: ScenarioResult[]) {
  ...
  for (const current of results) {
    ...
    const passChange = prev.passed !== current.passed
      ? (current.passed ? ' [PASS regained]' : ' [PASS lost]')
      : '';
    console.log(`  ${arrow}   ${current.name}  ${prev.avgScore.toFixed(2)} -> ${current.avgScore.toFixed(2)} (Δ${delta.toFixed(2)})${passChange}`);
  }
  console.log('');
}
