...
- 金标回归 `npm run eval` + 版本对比 `--baseline`
...
- `PROMPT_VERSION=v2 npm run dev` 一键切版本
...
├── docs/                        # RAG 知识源（npm run ingest 入库）
│   ├── rules/
│   ├── history/
│   └── products/
├── public/
...
```bash
npm install                       # 安装后端依赖
cd dashboard && npm install       # 安装 dashboard 依赖
cd ..

npm run qdrant:start              # 启动 Qdrant（可选）
npm run ingest                    # 首次或更新知识后入库
npm run build:dashboard           # 构建 React Dashboard 静态产物
npm run dev                       # 启动 Fastify（tsx watch）
```
...
生产构建：`npm run build` → `node dist/src/server.js`
...
每次改完 `docs/` 重新跑 `npm run ingest`，覆盖入库。
...
npm run eval -- --save v1-baseline   # 先用 v1 跑 baseline（如果还没存）

PROMPT_VERSION=v2 npm run eval -- --baseline v1-baseline
...
4. 跑 `npx tsc -p tsconfig.json --noEmit` 验证
...
```bash
npm run eval                     # 跑全部场景
npm run eval -- --filter happy   # 只跑名字含 "happy" 的
npm run eval -- --save v1-base   # 存当前结果
npm run eval -- --baseline v1-base   # 对比当前 vs baseline
```
...
```bash
npm run dev:dashboard            # Vite 开发模式，5173 端口，自动代理 API 到 3001
npm run build:dashboard          # 生产构建，输出到 public/dashboard/
```
