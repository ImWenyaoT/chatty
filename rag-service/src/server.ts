...
const chatRequestSchema = z
  ...
  .refine((data) => data.question?.trim() || data.imageUrl, {
...
app.get('/memory/:customerId', async (request, reply) => {
  ...
  const productId =
    typeof request.query === 'object' && request.query && 'productId' in request.query
...
app.post('/chat', async (request, reply) => {
  ...
  const recordedQuestion =
    [parsed.data.question ?? '', parsed.data.imageUrl ? `[图片] ${parsed.data.imageUrl}` : '']
      .filter(Boolean)
      .join(' ')
