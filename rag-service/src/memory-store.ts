...
function isInvalidSystemReview(review: {
  score: number
  source: string
  issues?: string[]
  suggestions?: string[]
...
function inferSizeRecommendation(
...
async function markOrderPlacedInternal(input: {
  ...
  const conversationProfileSummary = buildConversationProfileSummary(
    productMemory.conversationProfile,
  )
...
export async function getProductMemory(
  customerId: string,
  productId?: string,
  conversationId?: string,
) {
...
export async function getReviewSummary() {
  ...
  const byVersion = new Map<
    string,
    { version: string; count: number; scoreSum: number; lowScoreCount: number; errorCount: number }
  >()
  ...
  const promptVersions = Array.from(byVersion.values())
    .map((slot) => ({
...
async function appendConversationMemoryInternal(input: AppendConversationMemoryInput) {
  ...
  customerMemory.bodyProfiles = mergeBodyProfiles(
  ...
  if (reviewStatus) {
    ...
    productMemory.conversationProfile.reviewCheck = {
      ...
      passed,
      reviewedAt:
        passed || failed ? now : productMemory.conversationProfile.reviewCheck?.reviewedAt,
      source: 'manual',
  ...
  const priceQuote =
    inferPriceQuote(input.productId, now) ?? productMemory.conversationProfile.priceQuote
  ...
  if (productMemory.conversationProfile.orderPlacement?.orderNo) {
    productMemory.conversationProfile.orchestration = {
      ...productMemory.conversationProfile.orchestration,
      stage: 'post_order_followup',
      currentGoal: productMemory.conversationProfile.orderReadiness?.needReviewCheck
        ? '订单已提交，继续完成复核'
        : '跟进已下单客户',
      ...
      nextAction: productMemory.conversationProfile.orderReadiness?.needReviewCheck
        ? 'confirm_review'
        : 'close_loop',
      ...
      paused: false,
      replyTemplateKey:
        productMemory.conversationProfile.orchestration?.replyTemplateKey ?? 'post_order_followup',
      shouldUseRag: productMemory.conversationProfile.orchestration?.shouldUseRag ?? true,
      shouldUseBusinessTools:
        productMemory.conversationProfile.orchestration?.shouldUseBusinessTools ?? true,
  ...
  productMemory.recentMessages = [
    ...
    createMessage(
      ...
      input.question,
      now,
      input.userIntent ? { intent: input.userIntent } : undefined,
    ),
  ...
  scheduleReview({
    ...
    history: productMemory.recentMessages
      .slice(-10)
...
async function addReviewInternal(input: {
  customerId: string
  productId: string
  rating: number
  comment?: string
...
export async function reEvaluateConversation(input: {
  customerId: string
  productId?: string
  conversationId?: string
}) {
...
async function reEvaluateConversationInternal(input: {
  customerId: string
  productId?: string
  conversationId?: string
}) {
  ...
  const assistantReply = [...productMemory.recentMessages]
    .reverse()
  ...
  productMemory.reviews = (productMemory.reviews || []).filter(
    (review) => !isInvalidSystemReview(review),
  )

  const latestSystemReview = [...productMemory.reviews]
    .reverse()
    .find(
