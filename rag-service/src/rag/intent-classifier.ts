...
const FAST_PATH_RULES: Array<{ intent: UserIntent; pattern: RegExp; confidence: 'high' | 'medium' }> = [
  ...
  { intent: 'small_talk',      pattern: /^(?:在[吗么]?|您?好|你好|哈喽|hi|hello|hey|嗨|早|晚上好|早上好|中午好)[\s!?？。！~～]*$/i, confidence: 'high' },
  { intent: 'small_talk',      pattern: /^(?:谢谢|感谢|辛苦[了啦]?|多谢|thanks|thx|麻烦了)[\s!?？。！~～]*$/i, confidence: 'high' },
  { intent: 'confirm',         pattern: /^(?:是|是的|对|对的|嗯|嗯嗯|好的|好|行|可以|没错|没问题|就这样|ok|OK)[\s!?？。！~～]*$/, confidence: 'high' },
  { intent: 'request_handoff', pattern: /(?:要投诉|转人工|找人工|找店长|找经理|找客服|人工服务|需要人工)/, confidence: 'high' },
];
...
function classifyByKeywords(question: string, hasContext: boolean): IntentClassification {
  ...
  if (/图片|照片|样图|款式图|尺码表|价目表|价格表|链接|网址|看[下看]?一?下|多少钱|什么价|物流|包邮|几天到|退换|换货/.test(q)) {
  ...
  if (/[0-9]+月[0-9]+[日号]|[0-9]{4}[-/][0-9]{1,2}[-/][0-9]{1,2}|租[一二三四五六七八九十]?[0-9]?[几多]?天|档期|租赁时间|时间是|[0-9]+月|明[天日]|后[天日]|大后天|下周|下下周/.test(q)) {
  ...
  if (/(?:黑|白|蓝|灰|红|粉|紫|绿|金|银|棕|卡其|米|驼)色|双排扣|单排扣|三件套|两件套|燕尾|礼服|旗袍|晚礼|婚纱|SUIT[-_ ]?\d+|想租|要租|想要|看中/.test(q)) {
...
function buildContextLine(
  ...
  const parts: string[] = [];
  if (profile?.productIntent?.currentProductText) parts.push(`已锁定商品: ${profile.productIntent.currentProductText}`);
  if (profile?.rentalPeriod?.startDate) parts.push(`已有档期: ${profile.rentalPeriod.startDate} ~ ${profile.rentalPeriod.endDate ?? '?'}`);
  if (profile?.heightCm || profile?.weightKg) parts.push(`已有体型: ${profile.heightCm ?? '?'}cm / ${profile.weightKg ?? '?'}kg`);
...
export async function classifyUserIntent(input: {
  ...
  try {
    ...
    console.warn('[intent-classifier] LLM 调用失败，回退关键词:',
