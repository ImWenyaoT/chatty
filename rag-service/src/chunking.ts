...
import { KnowledgeChunk, SourceType } from './types.js';
...
function chunkQaCsv(filePath: string, content: string, sourceType: SourceType, title: string) {
  ...
  return lines.slice(1).map((line, index) => {
    ...
    if (columns.length !== 2) {
      throw new Error(`Invalid QA CSV row in ${filePath} at line ${index + 2}. Expected exactly 2 columns.`);
    }

    const question = columns[0].trim();
    const answer = columns[1].trim();
    if (!question || !answer) {
      throw new Error(`Invalid QA CSV row in ${filePath} at line ${index + 2}. Question and answer cannot be empty.`);
    }
