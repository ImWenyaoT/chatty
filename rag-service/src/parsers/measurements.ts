...
export function extractHeightWeightFromText(text: string) {
  const normalized = text
    ...
    .replace(/\s+/g, ' ')
    .trim();
  const heightMatch = normalized.match(/身高(?:是|为|改成|改为|修改成|更新为)?\s*([0-9]+(?:\.[0-9]+)?)\s*(?:cm|厘米)?/i);
  const weightMatch = normalized.match(/体重(?:是|为|改成|改为|修改成|更新为)?\s*([0-9]+(?:\.[0-9]+)?)\s*(kg|公斤|斤)?/i);
  ...
  if (!hasHeightLabel && !hasWeightLabel && !hasDateContext && (heightCm === undefined || weightKg === undefined)) {
    ...
    if (pairMatch) {
      ...
      const inferredUnit = pairMatch[4] || inferWeightUnit(pairMatch[3], pairMatch[2] ? undefined : '斤');
...
export function extractQuantityFromText(rawText: string): number | undefined {
  ...
  const text = rawText
    ...
    .replace(/\s+/g, '');
  // "数量X" / "数量是X" / "件数X"
  const explicitLabel = text.match(/(?:数量|件数)(?:是|:|=)?\s*([0-9一二两俩三四五六七八九十]{1,3})/);
  if (explicitLabel) {
    const n = toQuantityNumber(explicitLabel[1]);
    if (n !== undefined) return n;
  }
  // "X件" / "X套" / "X条" / "X身" / "X个" / "X对" + 量词；前面数字 1-99
  const measureWord = text.match(/(?:^|[^.0-9])([0-9]{1,2}|[一二两俩三四五六七八九十])\s*(件|套|条|身|个|对)/);
  if (measureWord) {
