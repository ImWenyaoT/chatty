...
describe("渲染 spec", () => {
  it("单商品用卡片，多商品自动切成对比表", () => {
    const one = buildRenderSpec([product("P1", "A")]);
    assert.equal(one?.elements["el-0"]?.type, "product_card");

    const many = buildRenderSpec([product("P1", "A"), product("P2", "B")]);
    assert.equal(many?.elements["el-0"]?.type, "product_table");
  });

  it("没有内容时返回 null，不渲染空容器", () => {
    assert.equal(buildRenderSpec([]), null);
  });
  ...
  it("spec 只使用 catalog 声明过的组件", () => {
    ...
    const spec = buildRenderSpec([product("P1", "A"), product("P2", "B")]);
