...
def test_parses_json_with_leading_prose() -> None:
    """模型在 JSON 前面写了说明文字时，仍要能把结构化部分捞出来。

    这是 2026-07-27 的 Pass^3 评估暴露的真实情况：
    提示词写了"只返回一个 JSON 对象"，但模型有时会先解释一句再给 JSON。
    Harness 不能因为模型不守规矩就整体失败。
    """
    raw = (
        "对 P012（绿联氮化镓 65W）来说，它体积小、功率足，适合出差携带。\n"
...
def test_parses_json_wrapped_in_code_fence_with_prose() -> None:
    """前有说明、后有代码块围栏的混合格式也要能解析。"""
    raw = (
        "好的，我的推荐如下：\n"
        "```json\n"
        '{"recommendations":[{"product_id":"P003","reason":"降噪适合通勤",'
        '"marketing_copy":"通勤路上安静下来"}]}\n'
        "```\n"
        "希望对你有帮助。"
    )
    draft = parse_agent_draft(raw)
