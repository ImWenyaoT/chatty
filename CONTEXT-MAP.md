...
- **Long-term Customer Memory** — explicit, stable, source-backed customer facts retained for future transactions. It is distinct from transient Context and from the shared Knowledge Base; it has no repeat-customer gate, candidate state, inference pipeline, confidence score, or vector index.
