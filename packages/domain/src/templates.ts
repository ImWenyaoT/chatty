...
// 改话术就改这里；答案结构 / 规则不变。
// 从 legacy rag-service/src/rag/templates.ts 平移，文案逐字不变（金标断言依赖）。

import type { Action } from './types.js'

/** 价格数字格式化：整数原样、小数保留一位（199 / 99.5） */
function formatPrice(value?: number) {
  if (value === undefined) return ''
  return Number.isInteger(value) ? String(value) : value.toFixed(1)
}

/** 档期区间格式化："起 到 止"；起止相同或只有一端时输出单日 */
...
/** 解析 "YYYY-M-D" 形式的日期文本，不合法返回 undefined */
...
/** 把一个 Action 实例渲染成最终客服回复文本（确定性模板策略） */
export function renderAction(action: Action): string {
  switch (action.kind) {
    ...
    case 'guide_order': {
      ...
      const priceText =
        action.dailyPrice !== undefined ? `首日 ${formatPrice(action.dailyPrice)} 元，` : ''
      const qty = action.quantity ?? 1
