...
// 从 legacy rag-service/src/conversation-orchestrator.test.ts 平移。
// 把「阶段推导优先级、slot 组合矩阵、主动追问上限、纯函数契约」固定下来，
// 改坏任何一条分支都会立刻红。
// 相对 legacy 的变更（docs/architecture.md §8.1）：post_order_followup 现在可达，
// 原先钉住「不可达」现状的 characterization 用例改为断言新行为，并新增专项用例。
...
test('pendingSlots 与 blockingIssues 跟随 readiness 缺口一一对应', () => {
  ...
  assert.deepEqual(result.blockingIssues, ['还没有锁定具体商品', '下单前还需要和用户复核关键信息'])
})
...
test('多个槽位同时缺失时 followUpQuestion 一次性问全，避免割裂追问', () => {
  const result = deriveConversationOrchestration({
    profile: makeProfile(),
    orderReadiness: makeReadiness({
      needRentalPeriod: true,
      needHeightWeight: true,
      needQuantity: true,
    }),
    productId: 'SUIT-001',
    now: NOW,
...
// ---------- §8.1 行为修复：post_order_followup 可达（close_loop 不再是死代码） ----------
// legacy 版本这里钉的是「orderPlaced 点亮但 stage 永远不会推导为 post_order_followup」的
// 现状缺口；重写按 docs/architecture.md §8.1 修复后，改为断言新行为。

test('§8.1：orderPlacement 存在且复核完成 → post_order_followup / close_loop', () => {
  const result = deriveConversationOrchestration({
    profile: makeProfile({
      orderPlacement: { orderNo: 'XY-20260702-001', placedAt: NOW, source: 'manual' },
      reviewCheck: { needed: true, completed: true, passed: true, source: 'system' },
  ...
  assert.equal(result.stage, 'post_order_followup')
  assert.equal(result.nextAction, 'close_loop')
  assert.equal(result.replyTemplateKey, 'post_order_followup')
  assert.equal(result.shouldUseRag, true)
  assert.equal(result.shouldUseBusinessTools, true)
  // close_loop 属于收尾动作，不再阻塞等待用户输入
  assert.equal(result.waitingForUser, false)
})

test('§8.1：售后跟进优先级最高——即使 readiness 仍报缺口也不回退到前置阶段', () => {
  const result = deriveConversationOrchestration({
    profile: makeProfile({
      orderPlacement: { orderNo: 'XY-20260702-002', placedAt: NOW, source: 'manual' },
      reviewCheck: { needed: true, completed: true, passed: true, source: 'system' },
    }),
    // 订单已落地后 readiness 缺口（如档期变更触发的重算）不应把用户拽回收集流程
    orderReadiness: makeReadiness({ needRentalPeriod: true }),
    productId: 'SUIT-001',
    now: NOW,
  })
...
test('§8.1：只下单未复核 / 只复核未下单，都不进入 post_order_followup', () => {
  // 只有 orderPlacement、复核未完成 → 仍按缺口走复核阶段
  const unreviewed = deriveConversationOrchestration({
    profile: makeProfile({
      orderPlacement: { orderNo: 'XY-20260702-003', placedAt: NOW, source: 'manual' },
      reviewCheck: { needed: true, completed: false, passed: false, source: 'system' },
    }),
    orderReadiness: makeReadiness({ needReviewCheck: true }),
    productId: 'SUIT-001',
    now: NOW,
  })
  assert.equal(unreviewed.stage, 'review_confirming')
  assert.equal(unreviewed.nextAction, 'confirm_review')

  // 只有复核完成、订单还没落地 → 引导下单，不能提前进入售后
  const notPlaced = deriveConversationOrchestration({
    profile: makeProfile({
      reviewCheck: { needed: true, completed: true, passed: true, source: 'system' },
    }),
    orderReadiness: makeReadiness({ readyToOrder: true }),
    productId: 'SUIT-001',
    now: NOW,
  })
  assert.equal(notPlaced.stage, 'order_guiding')
  assert.equal(notPlaced.nextAction, 'guide_order')
})
