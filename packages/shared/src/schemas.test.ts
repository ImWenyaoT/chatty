...
import { legacyChatInputSchema, traceReviewInputSchema } from './schemas.js'
...
test('traceReviewInputSchema：人工反馈标签、reviewer 和 tags 通过并归一化', () => {
  const result = traceReviewInputSchema.safeParse({
    traceId: 'tr_1',
    label: 'flagged',
    reviewer: 'pm',
    note: '需要补金标',
    tags: [' needs_golden ', 'needs_golden', 'policy_gap'],
  })

  assert.ok(result.success)
  assert.deepEqual(result.data.tags, ['needs_golden', 'policy_gap'])
})

test('traceReviewInputSchema：非法 label 和空 reviewer 被拒绝', () => {
  const result = traceReviewInputSchema.safeParse({
    traceId: 'tr_1',
    label: 'maybe',
    reviewer: '',
