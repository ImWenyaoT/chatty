...
export const DEVELOPMENT_METHOD_RULE =
  "功能实现必须先贴近 docs/jd.md 下限，再以 claude-code 作为该能力的唯一主参考（架构形状与复杂度上限同源）；调试必须用搭积木复现法收敛到最小可失败块，再把修复沉淀为自动化回归。";
...
export type ReferenceDebuggingMethod = {
  readonly allowedReferences: readonly ["claude-code"];
