...
// search_knowledge（docs/agentic-search-design.md §3）：LLM 主动检索店铺知识库的
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: 'string',
          description:
            '中文关键词或短语，2 到 8 个字为宜，例如"换码"、"押金"、"店铺电话"、"租期计算"。不要输入完整句子，不要带标点。',
        },
      },
      required: ['query'],
    },
...
function formatSearchResult(
  ...
  for (const [index, hit] of top.entries()) {
    const text =
      hit.text.length > MAX_HIT_CHARS ? `${hit.text.slice(0, MAX_HIT_CHARS)}……[已截断]` : hit.text
  ...
  if (hidden > 0) {
    parts.push(`（还有 ${hidden} 条未显示。如需更精确的结果，换更具体的关键词再搜一次。）`)
  }
  return { output: parts.join('\n\n'), matches: hits.length, truncated: clipped || hidden > 0 }
}
