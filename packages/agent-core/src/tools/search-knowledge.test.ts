// search_knowledge 工具的行为测试（docs/agentic-search-design.md B2 验收）：
...
function searcherOf(hits: Array<{ text: string; section: string }>): KnowledgeSearcher {
  return { search: () => hits }
}
...
test('命中数超过 top-k：头行计数、编号条目含出处、尾行提示剩余数量', async () => {
  ...
  const result = (await tool.execute({ query: '押金' })) as SearchKnowledgeResult
  assert.ok(result.output.startsWith('找到 5 条相关内容，显示最相关的前 3 条：'))
  assert.match(result.output, /\[1\] 来源：文档 › 小节1\n第 1 条正文内容/)
  assert.match(result.output, /\[3\] 来源：文档 › 小节3/)
  assert.ok(!result.output.includes('[4]'))
  assert.ok(
    result.output.endsWith('（还有 2 条未显示。如需更精确的结果，换更具体的关键词再搜一次。）'),
  )
...
test('命中数不超过 top-k：简化头行、无截断尾行', async () => {
  ...
  const result = (await tool.execute({ query: '换码' })) as SearchKnowledgeResult
...
test('空结果：显式文案 + 可执行建议，绝不回空串，且与失败可区分', async () => {
  ...
  const result = (await tool.execute({ query: '押金 多少' })) as SearchKnowledgeResult
  assert.equal(
    result.output,
...
test('单条正文超 800 字符：截到上限并加已截断后缀', async () => {
  ...
  const tool = createSearchKnowledgeTool(searcherOf([{ text: long, section: '文档 › 长文' }]))
...
test('结果总量到顶：停止追加条目（不截半条）并在尾行说明剩余', async () => {
  ...
  const hits = makeHits(3).map((hit) => ({ ...hit, section: '标'.repeat(1900) }))
...
test('工具元数据：risk=low、无需审批、参数 schema 要求 query', () => {
  ...
  assert.ok(tool.description.includes(`返回最相关的前 ${SEARCH_KNOWLEDGE_TOP_K} 条`))
})
...
test('policy 门：closed session 拒绝执行', async () => {
  ...
  await assert.rejects(
    () =>
      registry.invokeWithPolicy('search_knowledge', { query: '押金' }, createDefaultPolicy(), {
