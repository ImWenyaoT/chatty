...
test("compose step passes harness runtime to modelFn for SDK-owned tool orchestration", async () => {
  const result = await runCustomerServiceHarnessStep({
    ...
    modelFn: async (_prompt, runtime) => {
      ...
      assert.equal(
        runtime?.registry?.get("search_knowledge")?.name,
        "search_knowledge",
      );
...
test("compose step does not expose knowledge search loop for availability tasks", async () => {
  ...
  const result = await runCustomerServiceHarnessStep({
    ...
    toolLoopFn: async () => {
      loopCalls += 1;
      return {
        text: '{"action":"answer_question","reply":"不应该走搜索循环。"}',
      };
    },
  });
...
test("搜索循环：先搜后答，搜索调用落 trace toolCalls 与 knowledge fragment", async () => {
  ...
  const toolLoopFn: CustomerServiceToolLoopFn = async (messages, tools) => {
    ...
    if (seenMessages.length === 1) {
      ...
      return {
        toolCalls: [
          { id: "c1", name: "search_knowledge", arguments: '{"query":"押金"}' },
        ],
      };
    }
    return {
      text: '{"action":"answer_question","reply":"押金需按具体订单确认。"}',
    };
  };
  const result = await runCustomerServiceHarnessStep({
  ...
  const knowledge = result.trace.context.fragments.filter(
    (f) => f.kind === "knowledge",
  );
...
test("搜索循环：商品尺码问题会把泛词 query 收敛到当前 productId 的尺码事实", async () => {
  ...
  const toolLoopFn: CustomerServiceToolLoopFn = async () => {
    ...
    return {
      text: '{"action":"answer_question","reply":"您这个身高体重建议 L 码。"}',
    };
  };
  const result = await runCustomerServiceHarnessStep({
  ...
  assert.equal(result.trace.toolCalls.length, 1);
  assert.deepEqual(result.trace.toolCalls[0].arguments, {
    query: "SUIT-001 尺码",
  });
  assert.match(result.step.reply ?? "", /L 码/);
});
...
test("搜索循环：坏 tool_calls 参数收到重试文案且计入轮数，不触发检索", async () => {
  ...
  const toolLoopFn: CustomerServiceToolLoopFn = async (messages, tools) => {
    ...
    badArgsReplies = messages.flatMap((m) =>
      m.role === "tool" ? [m.content] : [],
    );
    return {
      text: '{"action":"answer_question","reply":"参数一直不对，只能直接回答。"}',
    };
  };
  const result = await runCustomerServiceHarnessStep({
    event: userEvent("租衣服要押金吗"),
