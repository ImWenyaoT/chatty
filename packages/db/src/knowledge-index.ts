 ...
 */
export function syncKnowledgeIndex(db: Db, knowledgeDir: string): KnowledgeSyncResult {
  const sources = listSourceFiles(knowledgeDir).map((file) => ({
    docId: relative(knowledgeDir, file).split(sep).join('/'),
    content: readFileSync(file, 'utf8'),
  }))
  const hash = createHash('sha1')
  for (const source of sources) hash.update(source.docId).update('\0').update(source.content)
  ...
  const chunks = sources.flatMap((source) => chunkKnowledgeFile(source.docId, source.content))
  ...
  db.transaction(() => {
    ...
    chunks.forEach((chunk, index) => {
      insert.run(index + 1, chunk.text, chunk.summary, chunk.docId, chunk.section, chunk.sourceType)
    })
    db.prepare(
...
export function chunkKnowledgeFile(docId: string, content: string): KnowledgeChunk[] {
...
function chunkMarkdown(
  ...
  for (const part of clean.split(/^(?=##\s)/m)) {
    ...
    const sectionTitle = firstLine.startsWith('## ') ? firstLine.slice(3).trim() : undefined
    ...
    for (const piece of splitByParagraph(trimmed)) {
      chunks.push({ text: piece, summary: makeSummary(section, piece), docId, section, sourceType })
    }
  }
  return chunks
}
...
function chunkQaCsv(
  ...
  const lines = content
    ...
    .map((line) => line.trim())
    .filter(Boolean)
  const header = lines[0] ? parseCsvLine(lines[0]).map((cell) => cell.toLowerCase()) : []
  ...
  return lines.slice(1).map((line, index) => {
    ...
    if (cols.length !== 2 || !cols[0] || !cols[1]) {
      throw new Error(`[knowledge-index] ${docId} 第 ${index + 2} 行必须恰好两列且均非空`)
    }
    const summary = `Q: ${cols[0]}`
    return { text: `${summary}\nA: ${cols[1]}`, summary, docId, section: docTitle, sourceType }
  })
}
