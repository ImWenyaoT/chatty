...
function ensureControlPlaneColumns(db: Db): void {
  const workflowColumns = new Set(
    (
      db.prepare("PRAGMA table_info(workflow_runs)").all() as { name: string }[]
  ...
  const queueColumns = new Set(
    (
      db.prepare("PRAGMA table_info(conversation_event_queue)").all() as {
        name: string;
      }[]
    ).map((column) => column.name),
  );
  if (!queueColumns.has("idempotency_key")) {
    db.exec(
      "ALTER TABLE conversation_event_queue ADD COLUMN idempotency_key TEXT",
    );
  }
  db.exec(
  ...
  const jobColumns = new Set(
    (
      db.prepare("PRAGMA table_info(background_jobs)").all() as {
        name: string;
      }[]
    ).map((column) => column.name),
  );
