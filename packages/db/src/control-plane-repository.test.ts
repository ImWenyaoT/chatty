...
test('workflow controller enforces one active run and idempotent replay', () => {
  ...
  assert.equal(control.transitionRun(first.id, 'completed').status, 'completed')
})
...
test('workflow lease expires, fences the old owner, and can be renewed by its owner', () => {
  ...
  const first = control.claimRun('run-lease', 'worker-old', '2026-07-11T00:00:00.000Z', 1_000)
  assert.equal(first?.leaseOwner, 'worker-old')
  assert.equal(
    control.heartbeatRun('run-lease', 'worker-old', '2026-07-11T00:00:00.500Z', 1_000),
    true,
  )
  assert.equal(control.claimRun('run-lease', 'worker-new', '2026-07-11T00:00:01.000Z'), undefined)
  const recovered = control.claimRun('run-lease', 'worker-new', '2026-07-11T00:00:02.000Z')
  ...
  assert.equal(
    control.transitionRun('run-lease', 'completed', undefined, 'worker-new').status,
    'completed',
  )
})
...
test('conversation queue preserves FIFO and deduplicates idempotent input', () => {
  const control = createControlPlaneRepository(openDatabase(':memory:'))
  const firstId = control.enqueueConversationEvent('conversation-1', 'event-1', { question: '一' })
  assert.equal(
    control.enqueueConversationEvent('conversation-1', 'event-1', { question: '重复' }),
    firstId,
  )
  control.enqueueConversationEvent('conversation-1', 'event-2', { question: '二' })
  assert.deepEqual(control.dequeueConversationEvent('conversation-1'), { question: '一' })
  assert.deepEqual(control.dequeueConversationEvent('conversation-1'), { question: '二' })
})

test('startup recovery releases an interrupted FIFO claim', () => {
  const control = createControlPlaneRepository(openDatabase(':memory:'))
  control.enqueueConversationEvent('conversation-restart', 'event-restart', { question: '恢复' })
  const claimed = control.claimConversationEvent('conversation-restart')
  assert.ok(claimed)
  assert.equal(control.claimConversationEvent('conversation-restart'), undefined)
  assert.equal(control.releaseInterruptedConversationEvents(), 1)
  assert.deepEqual(control.claimConversationEvent('conversation-restart')?.event, {
    question: '恢复',
  })
})
...
test('human handoff has a distinct state and explicit leased resume transition', () => {
  ...
  assert.equal(
    control.transitionRun('run-handoff', 'waiting_for_handoff', undefined, 'worker-old').status,
    'waiting_for_handoff',
  )
