...
const realKnowledgeDir = fileURLToPath(new URL('../../../knowledge', import.meta.url))
...
test('chunker: QA CSV 表头或行格式非法时抛错', () => {
  assert.throws(() => chunkKnowledgeFile('history/bad.csv', 'q,a\nx,y\n'), /question,answer/)
  assert.throws(() => chunkKnowledgeFile('history/bad.csv', 'question,answer\n只有一列\n'), /两列/)
})

test('chunker: 超 1200 字符的 section 按段落切分且不截断段落', () => {
  const para = `${'长'.repeat(700)}。`
  const chunks = chunkKnowledgeFile('rules/big.md', `# 大文档\n\n${para}\n\n${para}\n\n短尾段。\n`)
...
test('真实语料: 迁移后的 knowledge/ 可索引，检索关键事实可命中', () => {
  ...
  const deposit = repo.search('押金')
  assert.ok(deposit.some((hit) => hit.text.includes('根据具体商品和订单规则确认')))
  const exchange = repo.search('换码')
  assert.ok(exchange.some((hit) => hit.text.includes('免费补发') || hit.text.includes('更换一次')))
  const suit = repo.search('西装')
