...
import { createChatCompletionsAdapter, type ToolDefinition } from './chat-completions-adapter.js'

/** 构造只回放固定响应的 mock client，并捕获发出的请求参数供断言。 */
function mockClient(message: unknown, capture?: { params?: unknown }): OpenAI {
  return {
    chat: {
      completions: {
...
test('模型回复 tool_calls 时解析为 toolCalls 形态（arguments 保持原始字符串）', async () => {
  ...
  assert.deepEqual(result, {
    toolCalls: [{ id: 'call_1', name: 'search_knowledge', arguments: '{"query":"押金"}' }],
  })
})
...
test('模型回复纯文本时解析为 text 形态并去除首尾空白', async () => {
  ...
  const result = await adapter.completeWithTools([{ role: 'user', content: '在吗' }], [SEARCH_TOOL])
...
test('tools 为空数组时不带 tools 参数（有界循环收尾轮的形态）', async () => {
  ...
  const result = await adapter.completeWithTools([{ role: 'user', content: '收尾' }], [])
