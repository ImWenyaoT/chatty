...
const memorySchema = z
  .object({ facts: z.array(z.object({ key: z.string(), value: z.string() }).strict()) })
  .strict()
...
test(
  ...
  async () => {
    ...
    const runCompaction = createAgentsSdkStructuredRunner({
      ...
      input: 'Goal: rent a suit. Confirmed size L. Still need the delivery date.',
      model,
      modelName,
      outputType: compactionSchema,
      outputExample: '{"currentGoal":"rent a suit","unresolved":["delivery date"]}',
      toolChoice: 'none',
      maxTurns: 1,
      signal: AbortSignal.timeout(30_000),
    })
...
test(
  ...
  async () => {
    ...
    const runLongContext = createAgentsSdkToolLoopFn({
      instructions: 'Answer with only the final marker from the supplied context.',
    ...
    const output = await runLongContext(`${'bounded context line\n'.repeat(4_000)}FINAL_MARKER_726`)

    assert.match(output, /FINAL_MARKER_726/)
    assert.equal(telemetry.length > 0, true)
    assert.equal(telemetry[0].inputCacheHitTokens + telemetry[0].inputCacheMissTokens > 0, true)
    assert.equal(telemetry[0].totalTokens >= telemetry[0].outputTokens, true)
  },
)

test('DeepSeek beta honors AbortSignal cancellation', { skip: contractSkip }, async () => {
  ...
  const pending = runCancelled('Explain every operational detail. '.repeat(8_000))
  setTimeout(() => controller.abort(new Error('contract cancellation')), 25).unref()
  const watchdog = new Promise<never>((_resolve, reject) => {
    setTimeout(() => reject(new Error('DeepSeek cancellation contract timed out')), 30_000).unref()
  })
