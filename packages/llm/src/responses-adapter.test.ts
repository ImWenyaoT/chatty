...
test('completeJson parses JSON returned by the Responses API', async () => {
  const client = {
    responses: {
      create: async () => ({
        output_text: '{"actionClass":"ask_info","reason":"询问库存"}',
      }),
    },
  }
  ...
  const parsed = await adapter.completeJson<{ actionClass: string; reason: string }>([
    { role: 'user', content: '还有库存吗？' },
  ])
