...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ...
    memory:
      customerId && conversationId ? memory.snapshot({ customerId, conversationId }) : undefined,
  })
}
...
export async function POST(request: Request) {
  ...
  if (!control.getRun(body.runId)) return NextResponse.json({ error: 'not_found' }, { status: 404 })
  try {
    return NextResponse.json({ run: new HarnessRunController(control).cancel(body.runId) })
  } catch {
