...
export function buildConversationControlView(
  ...
  return {
    ...
    workflow: run
      ? {
          ...run,
          displayState: displayWorkflowState(run, leaseExpired),
          leaseExpired,
        }
      : {
          displayState: queueDepth > 0 ? "queued" : "unknown",
          leaseExpired: false,
        },
  ...
  return {
    ...
    metrics: {
      ...
      retryRate: jobHealth.attemptedJobs
        ? jobHealth.retriedJobs / jobHealth.attemptedJobs
        : null,
...
function displayWorkflowState(
  ...
  if (leaseExpired && ["running", "paused"].includes(run.status))
    return "recovering";
