...
test('conversation control view explains queue depth and an expired workflow as recovering', () => {
  ...
  control.enqueueConversationEvent('conversation-1', 'queued-1', { question: '下一条' })
...
test('conversation control view reports unknown instead of success without a run', () => {
  ...
  assert.deepEqual(view.workflow, { displayState: 'unknown', leaseExpired: false })
})

test('conversation control view reports queued when durable input awaits a run', () => {
  const control = createControlPlaneRepository(openDatabase(':memory:'))
  control.enqueueConversationEvent('conversation-queued', 'request-queued', { question: '等待' })
...
test('operations control view exposes job evidence, delivery and aggregate health', () => {
  ...
  const claimed = control.claimDueJob('worker-1', '2026-07-11T00:00:00.000Z', 60_000)!
  control.failJob('job-1', 'temporary', '2026-07-11T00:01:00.000Z', 'worker-1', claimed.claimFence)
  const reclaimed = control.claimDueJob('worker-2', '2026-07-11T00:01:00.000Z', 60_000)!
  ...
  control.finishJob('memory-1', 'succeeded_no_output', {}, 'worker-3', memory.claimFence)
...
test('aggregate health includes jobs beyond the operations display limit', () => {
  ...
  for (let index = 0; index < 101; index += 1) {
    ...
    control.finishJob(job.id, 'succeeded_no_output', {}, 'worker', job.claimFence)
  }
