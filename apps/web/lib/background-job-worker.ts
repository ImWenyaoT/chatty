...
export async function dispatchBackgroundJob(
  ...
  const job = options.control.claimDueJob(
    options.workerId,
    now().toISOString(),
    leaseMs,
  );
    ...
    if (job.type === "scheduled_followup") {
      ...
      options.control.completeFollowup(
        ...
        job.claimFence,
        {
          ...
          payload: {
            ...asObject(result.followup.payload),
            sourceJobId: job.id,
          },
      ...
      if (!result.consolidation)
        throw new Error("memory consolidation produced no commit result");
