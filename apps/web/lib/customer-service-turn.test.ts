...
test("resumeCustomerServiceHandoff reopens SQLite and completes through the public turn seam", async () => {
  ...
  assert.equal(
    resumedRepos.control.getRun(handedOff.runId)?.status,
    "completed",
  );
  assert.ok(
...
test("Customer Service Turn records non-destructive compaction failure evidence", async () => {
  ...
  assert.deepEqual(
    repos.control.latestCheckpoint("conversation-failed-compaction"),
    previous,
  );
  assert.equal(
    repos.control
      .listRunEvents(result.runId)
      .some((event) => event.type === "compaction_failed"),
    true,
  );
...
test("runCustomerServiceTurn drains busy conversation input in durable FIFO order", async () => {
  ...
  assert.equal(
    repos.control.dequeueConversationEvent("conversation-fifo"),
    undefined,
  );
});
...
test("runCustomerServiceTurn releases a queued input when automatic dispatch fails", async () => {
  ...
  assert.equal(
    (
      repos.control.claimConversationEvent("conversation-retry")?.event as {
        eventId: string;
      }
    ).eventId,
    "event-retry",
...
test("recoverCustomerServiceTurns re-enters an expired run after reopening temporary SQLite", async () => {
  const path = join(
    mkdtempSync(join(tmpdir(), "chatty-recovery-")),
    "control.sqlite",
  );
  const firstDb = openDatabase(path);
...
test("runCustomerServiceTurn re-enters multiple durable FIFO inputs through the real turn seam", async () => {
  const path = join(
    mkdtempSync(join(tmpdir(), "chatty-fifo-")),
    "control.sqlite",
  );
  const db = openDatabase(path);
  const repos = {
  ...
  for (const [index, question] of ["第二条", "第三条"].entries()) {
    repos.control.enqueueConversationEvent(
      "conversation-real-fifo",
      `queued-${index}`,
      {
  ...
  assert.equal(
    repos.control.claimConversationEvent("conversation-real-fifo"),
    undefined,
  );
  db.close();
});
...
test("runCustomerServiceTurn observes a durable cancel from another controller and fences side effects", async () => {
  const path = join(
    mkdtempSync(join(tmpdir(), "chatty-cancel-")),
    "control.sqlite",
  );
  ...
  while (!executionSignal)
    await new Promise((resolve) => setTimeout(resolve, 0));
  const firstCancel = new HarnessRunController(cancellerControl).cancel(
    "run-cancel",
  );
  const repeatedCancel = new HarnessRunController(cancellerControl).cancel(
    "run-cancel",
  );
  ...
  assert.deepEqual(
    await recoverCustomerServiceTurns({ repos: restartedRepos }),
    [],
  );
  assert.equal(
    restartedRepos.control.getRun("run-cancel")?.status,
    "cancelled",
  );
  restartedDb.close();
});
