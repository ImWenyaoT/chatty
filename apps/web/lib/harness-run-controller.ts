...
export class HarnessRunController {
  ...
  start(input: {
    ...
    try {
      const existing = this.repository.getRunByIdempotencyKey(
        input.idempotencyKey,
      );
      if (existing) {
        return {
          run: existing,
          signal: AbortSignal.abort("idempotent_replay"),
          replayed: true,
        };
      }
      ...
      const claimed = this.repository.claimRun(
        run.id,
        this.owner,
        new Date().toISOString(),
      );
      if (!claimed)
        throw new Error(`workflow run could not be claimed: ${run.id}`);
  ...
  transition(
    ...
    const failureKind =
      typeof details.failureKind === "string" ? details.failureKind : undefined;
    const run = this.repository.transitionRun(
      runId,
      status,
      failureKind,
      this.owner,
    );
    this.repository.appendRunEvent(runId, "state_changed", {
      status,
      ...details,
    });
  ...
  recover(runId: string): { run: WorkflowRun; signal: AbortSignal } {
    const controller = new AbortController();
    const run = this.repository.claimRun(
      runId,
      this.owner,
      new Date().toISOString(),
    );
  ...
  heartbeat(runId: string): boolean {
    const renewed = this.repository.heartbeatRun(
      runId,
      this.owner,
      new Date().toISOString(),
    );
    if (!renewed) {
      activeAbortControllers
        .get(runId)
        ?.controller.abort("workflow_lease_lost");
    }
    return renewed;
  }
  ...
  resumeHandoff(runId: string): { run: WorkflowRun; signal: AbortSignal } {
    ...
    const run = this.repository.resumeHandoff(
      runId,
      this.owner,
      new Date().toISOString(),
    );
    if (!run)
      throw new Error(`workflow handoff could not be resumed: ${runId}`);
    const controller = new AbortController();
    activeAbortControllers.set(runId, { controller, owner: this.owner });
    this.repository.appendRunEvent(runId, "handoff_resumed", {
      owner: this.owner,
    });
    return { run, signal: controller.signal };
  }
