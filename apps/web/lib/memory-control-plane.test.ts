...
import { createControlPlaneRepository, createMemoryRepository, openDatabase } from '@rental/db'
import { dispatchBackgroundJob, type BackgroundJobExecutors } from './background-job-worker'

const now = '2026-07-11T00:00:00.000Z'

/** Opens a real temporary SQLite store for memory control-plane integration tests. */
function stores() {
  const db = openDatabase(join(mkdtempSync(join(tmpdir(), 'chatty-memory-')), 'harness.sqlite'))
  return { control: createControlPlaneRepository(db), memory: createMemoryRepository(db) }
}

/** Builds executors with only the selected memory behavior enabled. */
function memoryExecutors(overrides: Partial<BackgroundJobExecutors>): BackgroundJobExecutors {
...
test('conversation extraction cooling coalesces turns and later facts schedule fresh work', () => {
  ...
  control.finishJob(claim.id, 'succeeded_no_output', {}, 'worker-1', claim.claimFence)
...
test('fenced extraction atomically preserves provenance, deduplicates replay, and records no-output', async () => {
  ...
  assert.equal(control.listMemoryCandidates('customer-1')[0].sourceTraceId, 'trace-1')
...
test('global consolidation lease reclaim fences stale owner and commits promotion with summary', () => {
  ...
  control.scheduleMemoryConsolidation({ id: 'consolidate-2', customerId: 'customer-2', now })
  ...
  assert.equal(control.listMemoryCandidates('customer-1')[0].status, 'promoted')
...
test('expired consolidation cannot commit and no-output preserves the existing summary', () => {
  ...
  assert.equal(
    control.completeMemoryConsolidation(
      ...
      'worker-1',
      claim.claimFence,
      { customerId: 'customer-1', globalSummary: '不应写入', promotedIds: [], prunedIds: [] },
      afterExpiry,
    ),
    false,
  )
  ...
  assert.equal(
    control.completeMemoryConsolidation(
      ...
      { customerId: 'customer-1', globalSummary: '', promotedIds: [], prunedIds: [] },
      afterExpiry,
    ),
    true,
  )
...
test('invalid consolidation rolls back candidate and summary changes', () => {
  ...
  assert.equal(control.listMemoryCandidates('customer-1')[0].status, 'candidate')
