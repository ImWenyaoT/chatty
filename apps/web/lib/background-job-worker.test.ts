...
function executors(
  ...
  return { memoryExtract: unexpected, memoryConsolidate: unexpected, scheduledFollowup }
}
...
test('reclaimed claimant fences stale completion and produces one atomic outbox delivery', async () => {
  ...
  assert.equal(control.listJobEvents('followup-1').at(-1)?.type, 'completion_rejected')
  assert.equal(
...
test('atomic follow-up completion rolls back its outbox insert when completion cannot commit', () => {
  ...
  assert.throws(() =>
    control.completeFollowup('rollback-followup', 'worker-1', claim.claimFence, {
      id: 'occupied-id',
      conversationId: 'conversation-1',
      runId: 'run-1',
      payload: {},
