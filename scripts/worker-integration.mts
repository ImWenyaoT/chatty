...
import { createControlPlaneRepository, openDatabase } from '../packages/db/src/index.ts'
...
try {
  ...
  assert.equal(job?.status, 'succeeded', 'one-shot worker did not complete the seeded job')
  assert.equal(job?.attempts, 1, 'one-shot worker must claim the seeded job exactly once')
  assert.equal(outbox.length, 1, 'one-shot worker must publish exactly one delivery')
  assert.equal(outbox[0].id, 'outbox:integration-followup')
  assert.equal(outbox[0].runId, job?.runId, 'job completion and outbox must commit one run')
  assert.equal((outbox[0].payload as Record<string, unknown>).sourceJobId, 'integration-followup')
  assert.deepEqual(
    verifyControl.listJobEvents('integration-followup').map((event) => event.type),
    ['scheduled', 'claimed', 'succeeded'],
  )
  verifyDb.close()
  console.log('worker integration: PASS (seeded job claimed, completed, and delivered once)')
} finally {
