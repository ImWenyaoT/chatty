...
> **历史归档（非当前操作指南）**：本文保留落地前的设计与验收记录，其中 legacy target、命令和语料文件可能已删除。当前行为以 [`../current-architecture.md`](../current-architecture.md) 和代码为准。

> 状态：设计定稿（feat/agentic-search 分支，历史记录）。
...
- 本文档新增于 feat/agentic-search 分支；采纳/放弃的依赖变更（qdrant 移除等）
  在 B5 落地时同步更新 [open-source-adoption.md](open-source-adoption.md) 与
  [loop-engineering-plan.md](loop-engineering-plan.md) §16 账本。
