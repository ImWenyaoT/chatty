...
- **进程模型**：FastAPI 成为唯一 HTTP 进程（127.0.0.1:8000）。开发时 Vite dev server 将 `/api/chatty` 代理到 FastAPI；生产与 CI smoke 由 FastAPI 直接伺服 `web/dist` 静态产物，保持单一 origin。禁止两个后端进程同时写同一 SQLite 文件。
