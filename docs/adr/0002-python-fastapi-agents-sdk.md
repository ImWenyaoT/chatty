...
当时 Chatty 使用 Python Backend 与 TypeScript Frontend。Backend 承担 Agent、Tool、Harness、
SQLite 和 FastAPI HTTP；Frontend 使用 React + Vite。当前目录与运行时见已取代本 ADR 的
[ADR 0003](0003-typescript-migration.md)。
...
- Backend 直接使用 OpenAI Agents SDK for Python 的 `Agent`、`Runner` 和 `function_tool`。
...
静态检查、确定性测试和 Frontend 构建。需要真实模型和费用的评测不放进默认门禁。
