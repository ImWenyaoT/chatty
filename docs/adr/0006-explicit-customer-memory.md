...
Chatty 的简化 MVP 只在 Model 主动选择 bounded Tool 时保存客户明确表达、且跨交易稳定的事实。Harness 从受信任的 HTTP Context 注入 `customer_id`，以当前 Trace 作为来源；Model 不能提供或覆盖客户与来源标识。每条 Memory 在 SQLite 中保留事实、客户、来源和创建时间，并只通过同一客户范围内的简单词法搜索 Tool 供以后 Session 使用。
